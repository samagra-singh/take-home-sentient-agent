...
/**
 * Server action to get project information from package.json.
 * This function reads the project's package.json file, extracts relevant information,
 * validates required fields, and returns it.
 */
export const getProjectInfo = async (): Promise<GetProjectInfoResponse> => {
  ...
  if (errors.length > 0) {
    throw new Error(`Validation failed for package.json fields: ${errors.join(', ')}`);
  }

  const apiResponse: GetProjectInfoResponse = {
    name: pkg.name!,
    version: pkg.version!,
